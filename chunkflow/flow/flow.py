...
@click.option('--corner-offset', type=click.INT, nargs=6, default=None, callback=default_none,
    help='adjust bounding box corner offset')
@click.option('--expand', type=click.INT, nargs=3, default=None, callback=default_none,
...
@click.option('--crop', type=click.INT, nargs=3, default=None, callback=default_none,
...
@click.option('--translate', type=click.INT, nargs=3, default=None, callback=default_none,
    help='translate the bounding box by this offset.')
@operator
def adjust_bbox(tasks, corner_offset: tuple, expand: tuple, expand_uniform: int, crop: tuple, crop_uniform: int, translate: tuple):
    """adjust the corner of bounding box."""
    for task in tasks:
        if task is not None:
            if sum(v is not None for v in [corner_offset, expand, expand_uniform, crop, crop_uniform, translate]) != 1:
            ...
            if corner_offset is not None:
                ...
                adjustment = (translate[0], translate[1], translate[2],
...
@click.option('--suffix', '-s', default='', type=str,
    help='the post part of result file path. Normally include file extension.')
@click.option('--mode', '-m',
...
def plugin(tasks, name: str, input_names: str, output_names: str, file: str, args: str):
    ...
    """
    plugin_op = Plugin.from_file(file, name=name)
    ...
    for task in tasks:
        if task is not None:
            ...
            if input_names is not None:
                ...
                inputs = []
            outputs = plugin_op(inputs, args=args)
...
@click.option('--volume-offset', '-s', type=click.INT, nargs=3, default=None,
    help='volume offset or start')
...
@click.option('--driver', '-b', type=click.Choice(['n5', 'local']), default='local',
    help='the storage backend.')
@click.option('--output-chunk-name', '-o', type=str, default=DEFAULT_CHUNK_NAME,
    help='output chunk name.')
@operator
def load_zarr(tasks, store: str, path: str, chunk_start: tuple, volume_offset: tuple, voxel_size: tuple,
        chunk_size: tuple, driver: str, output_chunk_name: str, layer_type: str):
    """Load Zarr arrays."""
    if driver in ['local'] :
        store = zarr.storage.LocalStore(store)
    ...
    zdata = zarr.open_array(store, mode='r', path=path)
    attrs = zdata.attrs.asdict()

    # Note that this is the physical
    if volume_offset:
        volume_offset = Cartesian.from_collection(volume_offset)
    ...
    for task in tasks:
        if task is not None:
            if chunk_size is None and chunk_start is None and 'bbox' not in task:
                ...
                voxel_offset = arr_start + volume_offset  #chunk_start
            if voxel_offset == Cartesian(0, 0, 0):
                print('no voxel offset, set default value: 0x0x0')
            chunk = Chunk(arr, voxel_offset=voxel_offset, voxel_size=voxel_size, layer_type=layer_type)
            task[output_chunk_name] = chunk
        yield task


def _open_zarr(store: str, path: str, kwargs: dict, order: str, scale=None, timeout: float = 30.0, initialization=False):
    ...
    # It's possible that the zarr store is being created in another process, retry a few times if this fails
    start = time()
    delay_unit = 0.5
    retries = 0
    za = None
        ...
        try:
            if os.path.exists(store):
                if not initialization:
                    open_kws = kwargs.copy()
                    if path:
                        open_kws['path'] = path
                    za = zarr.open_array(store, **open_kws)
            else:
                if (
                        'SLURM_JOB_ID' not in os.environ or 'DISBATCH_REPEAT_INDEX' not in os.environ
                        or os.environ['DISBATCH_REPEAT_INDEX'] == '0'
                ):
                    os.makedirs(os.path.dirname(store), exist_ok=True)
                    create_kws = kwargs.copy()
                    create_kws.pop('mode', None)
                    if path:
                        create_kws['path'] = path
                    za = zarr.create(store=store, **create_kws)
                    ...
                    print(f'created zarr store: {store} with kwargs: {create_kws}')
                else:
                    raise FileNotFoundError('Waiting for zarr store to be created')
            break
...
def _save_zarr(za, path: str, zarr_kwargs: dict, chunk_array: np.ndarray, chunk_sub_bbox: BoundingBox, chunk_real_bbox: BoundingBox, order: str):
    if order not in ('xyz', 'zyx'):
        raise ValueError(f'invalid order: {order} (should be xyz or zyx)')
    order_slice = slice(None, None, -1) if order == 'xyz' else slice(None)
    ...
    if any(s < 0 for s in za_bbox.start) or any(e > s for e, s in zip(za_bbox.shape, za_shape)):
        print(f'chunk bbox {chunk_sub_bbox} is out of zarr array {za.shape}')
        return
    ...
    try:
        za[za_slices] = chunk_array
        # print(f'saved chunk to zarr: shape={chunk_array.shape}, offset={za_bbox.start}')
    except Exception as e:
        print(f'Error saving chunk to zarr: {e}', flush=True)
        print(chunk_array.shape, za_slices, za.shape, (chunk_array > 0).sum(), flush=True)
        raise e


def _resize_shape(shape, chunk_size, ignore_unaligned: bool):
    if chunk_size and any(m % n != 0 for m, n in zip(shape, chunk_size)):
        if ignore_unaligned:
            shape = tuple([m - m % n for m, n in zip(shape, chunk_size)])
        else:
            shape = tuple([m + (n - m % n) if (m % n) != 0 else m for m, n in zip(shape, chunk_size)])
    return shape


@main.command('create-zarr')
@click.option('--path', '-p', type=str, default=None,
    help = 'Zarr path in the store')
...
@click.option('--ignore-unaligned/--no-ignore-unaligned', default=True,
...
@click.option('--compression/--no-compression', default=True,
    help='use compression or not.')
@click.option('--timeout', type=click.FLOAT, default=30.0,
    help='timeout in seconds for opening zarr store.')
@operator
def create_zarr(tasks, store: str, path: str, shape: tuple, resolution: tuple, mip: int, dtype: str, chunk_size: tuple,
                ignore_unaligned: bool, order: str, compression: bool, timeout: float):
    order = order.lower()
    if order not in ('xyz', 'zyx'):
        raise ValueError(f'invalid order: {order} (should be xyz or zyx)')
    order_slice = slice(None, None, -1) if order == 'xyz' else slice(None)
    ...
    shape = _resize_shape(shape, chunk_size, ignore_unaligned)
    ...
    for task in tasks:
        if task is not None:
            za = _open_zarr(store, path, kwargs, order, scale, timeout, initialization=True)
        yield task
...
@click.option('--dtype', type=str, default=None,
    help='data type of the array.')
@click.option('--infer-dtype/--no-infer-dtype', default=True,
    help='infer the data type if it is not specified.')
@click.option('--chunk-size', '-c', type=click.INT, nargs=3,
    default=None, callback=default_none,
    help='Zarr chunk size.')
@click.option('--ignore-unaligned/--no-ignore-unaligned', default=False,
    help='crop the extra data in the zarr array; expand if False.')
    ...
    for task in tasks:
        if task is not None:
            ...
            if shape is None:
                ...
                shape_ = shape
            if chunk_size is None:
                chunk_size_ = chunk.shape
            else:
                chunk_size_ = chunk_size
            shape_ = _resize_shape(shape_, chunk_size_, ignore_unaligned)

            kwargs = {'mode': 'a'}
            assert shape_ is not None
            shape_ = shape_[order_slice]
            kwargs['shape'] = shape_
            if dtype is not None:
                kwargs['dtype'] = dtype
            elif infer_dtype and chunk.dtype is not None:
                kwargs['dtype'] = str(chunk.dtype)
            if chunk_size_ is not None:
                chunk_size_ = chunk_size_[order_slice]
                kwargs['chunks'] = chunk_size_
            if not compression:
                kwargs['compressors'] = None
            ...
            if shape is None:
                chunk_bbox_ = BoundingBox(Cartesian(0, 0, 0), Cartesian.from_collection(chunk.shape))
            else:
                chunk_bbox_ = chunk.bbox

            if split_chunks and any(m > n for m, n in zip(chunk.shape, chunk_size_)):
                chunk_bboxes = chunk_bbox_.decompose(chunk_size_, ignore_unaligned=ignore_unaligned)
            else:
                chunk_bboxes = [chunk_bbox_]

            # Crop chunk array if it extends past zarr array size
            chunk_slices = []
            za_shape = za.shape[order_slice]
            for i in range(chunk.ndim):
                if (i == 0 and chunk.ndim == 4) or shape is None:
            ...
            if chunk_array.size == 0:
                print(f'chunk/zarr overlap array is empty for {chunk.bbox} chunk')
                continue
...
@operator
def load_npy(tasks, name: str, file_path: str, resolution: tuple, output_name: str):
    """Read NPY files."""
    for task in tasks:
        if task is not None:
            start = time()
...
@operator
def load_json(tasks, name: str, file_path: str, output_name: str):
    """Read JSON file."""
    for task in tasks:
        if task is not None:
            start = time()
...
@click.option('--voxel-size', '-s', type=click.INT, nargs=3, default=None, callback=default_none,
              help='physical size of voxels. The unit is assumed to be nm.')
@click.option('--layer-type', '-l', type=str, default=None,
              help='the layer type in neuroglancer for visualization.')
@click.option('--dtype', '-d',
...
def mask(tasks, name, input_names: str, output_suffix: str, volume_path: str,
    ...
    """
    mask_op = MaskOperator(volume_path,
                           mip,
                           state['mip'],
                           inverse=inverse,
                           fill_missing=fill_missing,
...
def aggregate_skeleton_fragments(tasks, name, input_name, prefix, fragments_path, output_path):
    ...
    skel_op = AggregateSkeletonFragmentsOperator(fragments_path, output_path)
    if prefix:
        skel_op(prefix)
    else:
        for task in tasks:
...
def napari(tasks, name, voxel_size, inputs):
    ...
    napari_op = NapariOperator(
        name=name, voxel_size=voxel_size)
    for task in tasks:
        if task is not None:
            napari_op(task, selected=inputs)
        yield task
...
def view(tasks, name, image_chunk_name, segmentation_chunk_name):
    ...
    view_op = ViewOperator(name=name)
    for task in tasks:
        if task is not None:
